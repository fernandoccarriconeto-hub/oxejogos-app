...
export default function GamePage() {
    ...
    const initialize = async () => {
      ...
      if (session.status === 'theme_selection') {
        ...
        if (activeRound) {
          ...
          if (activeRound.status === 'answering') {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', {
        ...
        // Question comes ready from DB — show it immediately
        setCurrentScreen('question');
        const deadline = new Date(newRound.answer_deadline || '').getTime();
        const remaining = Math.max(0, Math.floor((deadline - Date.now()) / 1000));
      ...
      .on('postgres_changes', {
        ...
        if (updatedRound.status === 'answering') {
          setCurrentScreen('question');
  ...
  // Handle theme selection — fetch question from DB then create round instantly
  const handleSelectTheme = async () => {
    if (!selectedTheme || !gameSession || !user) return;

    setCurrentScreen('waiting_question');

    // Step 1: Fetch question from database (instant — no AI)
    let questionText = 'Qual é a capital do Brasil?';
    let correctAnswer = 'Brasília';
    let creativeAnswer = 'São Paulo, por ser a maior cidade';
    ...
    try {
      ...
      if (res.ok) {
        ...
        creativeAnswer = data.creativeAnswer;
      }
    } catch (error) {
      console.error('Error fetching question:', error);
    }

    // Step 2: Create round already with the real question (single INSERT, no UPDATE needed)
    const { data: roundData, error: roundError } = await supabase
      ...
      .insert({
        ...
        ai_correct_answer: correctAnswer,
        ai_creative_answer: creativeAnswer,
        status: 'answering',
